...
int main(){
	char *user_string=NULL,*sentence=NULL,*kbr,*file,*forget_string,*point,*answer,*keyword,*printConcept,*printTimesUsed;
	...
	do{
		...
		if(kbr){
			...
			if(keyword){
				...
				if (ptr != NULL){
					reply(questions,6);
					printf("The current is : %s",ptr->concept);
...
gtpList* answerTheQuestion(char* keyword,gtpList* prevSearch){
	...
	if (prevSearch == NULL){
		...
		while(current!=NULL){
			if(strcmp(current->sentence,keyword)==0){
				//printf("%s %s\n",GPT,current->sentence);
				current->timesUsed++;
				//printf("%s %d\n",GPT,current->timesUsed);
