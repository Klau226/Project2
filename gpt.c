...
void reply(char* concept,int search);					// Making the answers of ChatGpt
char* removeWhitespace(char* user_string);				// Removing whitespaces so we can search and forget without a problem
void learn(char* user_string);						// Taking the concept and the sentence from the user
char* learn_kbr();							// Allocating memory for the string
void insert_kbr(char* concept,char* sentence,int used,char* type);	// Inserting the concept/sentence in our list
void sortlist();								// Sorting the list
int search(char* stringToSearch); 					// Making sure that this concept is not in the list
void list_display();							// Prints all the concepts and sentences
void listdisplay_concept();						// Prints only the concept
void listdisplay_timesUsed();						// Prints the concept/times_used
void display_struct();							// Prints all the struct
void learn_file(char* file_string);					// Learning from a file
void forget(char* user_string);						// Taking the string that the user wants to delete
void deleteNode(gtpList *del);						// Deleting the node
char* question(char *user_string);					// Taking the string tha the user wants to search
gtpList* answerTheQuestion(char* keyword,gtpList* prevSearch);		// Searching the string
...
int main(){
	...
	do{
		...
		if(kbr){				// if "learn this >"
			learn(user_string);		
			sortlist();
			
		}
		else if(file){			// if "read this >"
			learn_file(user_string);
			sortlist();
		}
		else if(forget_string){		// if "forget this >"
			forget(user_string);
		}
...
void learn_file(char* file_string){
	FILE *fptr;
	char* token = NULL;
	char sstring1[256]={"/home/klaudio/Downloads/"};		// doesn't work without this
	char* variables[1];
	char* variables2[2];
	char* line;
	int i;
...
gtpList* answerTheQuestion(char* keyword,gtpList* prevSearch){
	gtpList *current = NULL;
    	...
    	if(prevSearch == NULL){
			...
			else if(strcmp(current->concept, keyword)){
				current->timesUsed++;
            			return current;
			} 
			...
			else if(strcmp(current->sentence, keyword)){
				current->timesUsed++;
            			return current;
			} 
        		if (current->next != NULL) {
        ...
        while (current != NULL) {
        	if (strstr(current->concept, keyword) != NULL) {
                	...
                	if (current->prev != head) {
                    		printf("Going Left! \n");
                    		current = current->prev;					//Partially works..It works in some cases but sometimes returns segmetation fault
        		}
        	}
        	if(strstr(current->sentence, keyword) != NULL){
                	current->timesUsed++;
