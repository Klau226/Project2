...
void deleteFirst(gtpList *current);

int main(){
	char *user_string=NULL,*sentence=NULL,*kbr,*file,*forget_string;
	...
	while( token != NULL ) {
   	...
   	i=0;
	   	...
	   	if(head == NULL) {  
	    	...
	    	current = head;    
	    	/*while(strcmp(temp->concept,variables[0])!=0) { 
	    		if(temp -> next == NULL) {  
				printf("\nCan't delete\n");  
			}
			temp = temp -> next;    
...
void deleteFirst(gtpList *current){
	if (current != NULL) {
    	...
    	//tmp->next->prev = NULL;
    	//free(tmp);
    	//printf("Deleted the first node in the list.\n");
}
